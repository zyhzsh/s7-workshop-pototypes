  ...
  return (
    <Link to={`/challenges/${record?.challenge_id}/records/${record?.id}`}>
      <Card
        ...
        <img src={record?.challenges?.img_url} className="absolute top-0 left-0  " />
        <div className='z-0'>
          <Typography variant="h2" >{record?.challenges.name}</Typography>
          <Chip variant="gradient" className='p-2' color='red' value={record?.challenges?.category || ""} />
          <Typography variant="h6">
            Checkpoin - {record?.checkpoints?.position}: </Typography>
          <Typography variant="h6">{record?.checkpoints?.name}</Typography>
